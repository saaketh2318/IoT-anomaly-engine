...
st.sidebar.title("IoT Streaming Simulation (Fast Debug Mode)")
...
if 'hst_model' not in st.session_state:
...
def generate_iot_data(anomaly_rate):
    ...
    if np.random.rand() < anomaly_rate or simulate_button:
        temperature += np.random.uniform(50, 100)
        humidity += np.random.uniform(80, 120)
        vibration += np.random.uniform(30, 50)
...
# Static Scaling
features_scaled = {
    'temperature': (features['temperature'] - 20) / (150 - 20),
    'humidity': (features['humidity'] - 30) / (200 - 30),
    'vibration': (features['vibration'] - 0) / (100 - 0),
}

# Diagnostic sidebar print
st.sidebar.markdown(f"**Generated Features:** {features}")
st.sidebar.markdown(f"**Static Scaled Features:** {features_scaled}")
...
if len(st.session_state.detected_flags) % 10 == 0:
...
st.title("🔍 Self-Healing IoT Fast Debug Mode")
